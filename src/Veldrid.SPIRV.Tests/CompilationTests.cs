...
public class CompilationTests
{
    ...
    public void VertexFragmentSucceeds(string vs, string fs, CrossCompileTarget target)
    {
        ...
        VertexFragmentCompilationResult result = SpirvCompilation.CompileVertexFragment(
            ...
            fsBytes,
            target,
            new CrossCompileOptions { Specializations = specializations }
        );
    ...
    [Theory]
    [InlineData("instance.vert", ShadercShaderKind.Vertex)]
    [InlineData("instance.frag", ShadercShaderKind.Fragment)]
    [InlineData("planet.vert", ShadercShaderKind.Vertex)]
    [InlineData("planet.frag", ShadercShaderKind.Fragment)]
    [InlineData("starfield.vert", ShadercShaderKind.Vertex)]
    [InlineData("starfield.frag", ShadercShaderKind.Fragment)]
    [InlineData("texel-fetch-no-sampler.frag", ShadercShaderKind.Fragment)]
    [InlineData("simple.comp", ShadercShaderKind.Compute)]
